 ...
 */
public final class Saturday extends LinearLayout {
	public final RecyclerView recyclerView;
	public final TextView dayTitle;

	public Saturday(final Context context) {
		this(context, null, 0);
	}

	public Saturday(final Context context, final AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public Saturday(final Context context, final AttributeSet attrs, final int defStyleAttr) {
		...
		List<Model> noteList = databaseHelper.getDayNotes(NotesTable.NAME,userName, 5);

		recyclerView = (RecyclerView) findViewById(R.id.recyclerView);
		dayTitle = (TextView) findViewById(R.id.dayTitle);
		dayTitle.setText("Saturday");
		...
		ContentAdapter contentAdapter = new ContentAdapter(context, items , 5);
	...
	private List<ContentAdapterModel> getItems(final List<Model> list) {
		...
		return new LinkedList<ContentAdapterModel>() {{
			...
			for (int i = 0; i < 24 && null != list; i++) {
				...
				add(new ContentAdapterModel(R.layout.content_cell, 5,i, row.note));
			}
		}};
	}
}