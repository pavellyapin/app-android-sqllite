 ...
 */
public final class Friday extends LinearLayout {
	...
	public Friday(final Context context) {
		this(context, null, 0);
	}

	public Friday(final Context context, final AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public Friday(final Context context, final AttributeSet attrs, final int defStyleAttr) {
		...
		List<Model> noteList = databaseHelper.getDayNotes(NotesTable.NAME,userName, 4);
		...
		ContentAdapter contentAdapter = new ContentAdapter(context, items ,4);
		...
		return new LinkedList<ContentAdapterModel>() {{
			...
			NotesTable.Row row = null;
				...
				for (int i = 0; i < 24 && null != list; i++) {
					...
					add(new ContentAdapterModel(R.layout.content_cell, 4, i, row.note));
				}
