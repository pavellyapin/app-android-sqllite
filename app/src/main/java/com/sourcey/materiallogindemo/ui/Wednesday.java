 ...
 */
public final class Wednesday extends LinearLayout {
	public final RecyclerView recyclerView;
	public final TextView dayTitle;

	public Wednesday(final Context context) {
		this(context, null, 0);
	}

	public Wednesday(final Context context, final AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public Wednesday(final Context context, final AttributeSet attrs, final int defStyleAttr) {
		...
		List<Model> noteList = databaseHelper.getDayNotes(NotesTable.NAME,userName, 2);

		recyclerView = (RecyclerView) findViewById(R.id.recyclerView);
		dayTitle = (TextView) findViewById(R.id.dayTitle);
		dayTitle.setText("Wednesday");
		...
		ContentAdapter contentAdapter = new ContentAdapter(context, items , 2);
	...
	private List<ContentAdapterModel> getItems(final List<Model> list) {
		...
		return new LinkedList<ContentAdapterModel>() {{
			...
			for (int i = 0; i < 24 && null != list; i++) {
				...
				add(new ContentAdapterModel(R.layout.content_cell, 2,i, row.note));
			}
		}};
	}
}