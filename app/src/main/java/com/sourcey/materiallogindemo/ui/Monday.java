 ...
 */
public final class Monday extends LinearLayout {
	public final RecyclerView recyclerView;
	public final TextView dayTitle;

	public Monday(final Context context) {
		this(context, null, 0);
	}

	public Monday(final Context context, final AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public Monday(final Context context, final AttributeSet attrs, final int defStyleAttr) {
		super(context, attrs, defStyleAttr);

		//((Application) this.getApplication()).getUser();
		...
		List<Model> noteList = databaseHelper.getDayNotes(NotesTable.NAME,userName, 0);
		//context.
		...
		ContentAdapter contentAdapter = new ContentAdapter(context, items , 0);
		...
		return new LinkedList<ContentAdapterModel>() {{
			...
			NotesTable.Row row = null;
				...
				for (int i = 0; i < 24 && null != list; i++) {
					...
					add(new ContentAdapterModel(R.layout.content_cell, 0, i, row.note));
				}
