 ...
 */
public final class Tuesday extends LinearLayout {
	public final RecyclerView recyclerView;
	public final TextView dayTitle;

	public Tuesday(final Context context) {
		this(context, null, 0);
	}

	public Tuesday(final Context context, final AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public Tuesday(final Context context, final AttributeSet attrs, final int defStyleAttr) {
		...
		List<Model> noteList = databaseHelper.getDayNotes(NotesTable.NAME,userName, 1);

		recyclerView = (RecyclerView) findViewById(R.id.recyclerView);
		dayTitle = (TextView) findViewById(R.id.dayTitle);
		dayTitle.setText("Tuesday");
		...
		ContentAdapter contentAdapter = new ContentAdapter(context, items , 1);
	...
	private List<ContentAdapterModel> getItems(final List<Model> list) {
		...
		return new LinkedList<ContentAdapterModel>() {{
			...
			for (int i = 0; i < 24 && null != list; i++) {
				...
				add(new ContentAdapterModel(R.layout.content_cell, 1,i, row.note));
			}
		}};
	}
}